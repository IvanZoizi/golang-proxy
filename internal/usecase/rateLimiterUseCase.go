...
import (
	...
	ip2 "github.com/IvanZoizi/ip-utils-golang"
	"proxy/internal/entity"
	"proxy/internal/repository"
)

type RateLimiterUseCase interface {
	GetRateLimitConfig() (*entity.RateLimitConfig, error)
	CheckRequestLimit(ip string) (bool, string, error)
	CheckTrafficLimit(ip string, downloadBytes, uploadBytes int64) (bool, string, error)
	CheckConnectionLimit(ip string, increment bool) (bool, string, error)
	GetSubnetLimits(ip string) (*entity.SubnetRateLimit, error)
...
func (uc *RateLimiterUseCaseImpl) GetRateLimitConfig() (*entity.RateLimitConfig, error) {
	return uc.repo.GetConfig()
}

func (uc *RateLimiterUseCaseImpl) getCachedData(ip string) *entity.RateLimitData {
	uc.cacheMu.RLock()
...
func (uc *RateLimiterUseCaseImpl) saveCachedData(ip string, data *entity.RateLimitData) {
	uc.cacheMu.Lock()
	uc.cache[ip] = data
	uc.cacheMu.Unlock()
...
func (uc *RateLimiterUseCaseImpl) periodicCacheSave() {
...
func (uc *RateLimiterUseCaseImpl) CheckRequestLimit(ip string) (bool, string, error) {
...
func (uc *RateLimiterUseCaseImpl) CheckTrafficLimit(ip string, downloadBytes, uploadBytes int64) (bool, string, error) {
	config, err := uc.repo.GetConfig()
	if err != nil {
		return true, "", err
	}
...
func (uc *RateLimiterUseCaseImpl) CheckConnectionLimit(ip string, increment bool) (bool, string, error) {
	config, err := uc.repo.GetConfig()
	if err != nil {
		return true, "", err
	}
...
func (uc *RateLimiterUseCaseImpl) resetCountersIfNeeded(data *entity.RateLimitData, now time.Time) {
...
func (uc *RateLimiterUseCaseImpl) blockIP(data *entity.RateLimitData, reason string, duration time.Duration) {
	data.IsBlocked = true
	data.BlockedUntil = time.Now().Add(duration)
	data.BlockedReason = reason
}

func (uc *RateLimiterUseCaseImpl) GetSubnetLimits(ip string) (*entity.SubnetRateLimit, error) {
	config, err := uc.repo.GetConfig()
	if err != nil {
		return nil, err
	}
...
func (uc *RateLimiterUseCaseImpl) GetIpsByRequest() ([]string, map[string]int) {
